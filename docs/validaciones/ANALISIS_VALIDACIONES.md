...
> **Origen: Función 3 del brief del cliente** ("Cuadro de mandos: diagnóstico por proveedor/seller", págs. 29-34). El cliente lo plantea como **un único informe por proveedor** con **cabecera + 4 bloques** (1·Impacto por tipo de error, 2·Matriz de trabajo, 3·Guías y referencias, 4·Impacto en negocio), marcando el **Owner** de cada fila (Seller = completitud de dato + imágenes; TIP = incongruencias de coherencia + estructura de designación). **Nosotros lo ordenamos mejor: partimos ese informe en dos exports disjuntos por owner** — la unión de los dos ≈ el informe único del cliente.

**Principio de partición por owner:** cada export lleva **solo lo que corrige ese owner** (no se solapan):
- **Export del seller** = **solo Owner: Seller** (completitud de dato + imágenes). **Sin material de referencia** (nada de guías completas): principio *"solo le decimos el cambio a realizar"*. La **regla concreta** que incumple va **embebida inline** junto a cada corrección (p. ej. *"faltan imágenes → categorías de imagen con su mín-máx, p. ej. Packshot; nombrado «Referencia-p-01»"*, *"estructura de título: [Marca]+[Modelo]+[Característica]+[Color]"*). *(El estándar de imágenes sale de la **guía eMerch** —tipo + nº mínimo—, no del brief: el #FFFFFF/1500px del brief no está en las guías, ver §pág.7 de dudas/respuestas.)* Las guías completas se quedan **internas** (módulo de Guías del toolkit).
- **Export interno TIP** = **solo Owner: TIP** (incongruencias de coherencia + estructura de designación). **Sí** puede incluir **material de referencia** (Bloque 3: enlaces a guía eMerch de la categoría, estándar de imágenes, estructura de título, atributos básicos/específicos).
- *(Aunque los exports sean distintos, **el equipo de Leroy tiene acceso a ambos**.)*
...
- **Informe del seller/proveedor** (hacia fuera): **PDF minimalista y defensivo** — solo **lo Owner: Seller** de **sus** referencias (completitud + imágenes), con la regla concreta inline; **sin** material de referencia, **sin** coherencia interna, **sin** estrategia de familia — **+ CSV/XLSX** de su matriz. El **CSV** incluye: **SKU · nombre del proveedor/seller · identificación 1P/3P · atributos sin completitud de dato**. *(No hay "PIM" — era un error de su slide.)*
- **Informe interno TIP / equipo ecommerce**: **solo Owner: TIP** (coherencia + estructura de designación), con el detalle para corregir o enrutar (1P → TIP, 3P → Marketplace) y **material de referencia** (guías/estándares). *(No incluye los ítems del seller: esos van en el export del seller, al que el equipo de Leroy también accede.)*
- **Bloque 4 · Estimación de impacto en negocio** (págs. 33-34): **🟢 verde** — depende de **Función 4 (Data-to-Revenue)**; las cifras del brief (+18% CR, −14% devoluciones) son "valores de referencia a validar con Digital Analytics". **Fuera del MVP** (o como bloque provisional claramente marcado).

**Orden de las imágenes — fuera de alcance (decidido).** La validación **no comprueba el orden** de las imágenes: la asunción es que **no nos llega el dato** (recibimos los **totales agregados por categoría de imagen**, no la secuencia). Encaja con la respuesta del cliente — *"el orden de las imágenes no hace falta"* (`Dudas y repsuestas doc validaciones.pdf`, pág. 6, bloque *"Pág 32. Guías y referencias"*) —, dicha en una lista sobre **qué comunicar al seller y qué no**. Consecuencia para el PDF: **no se comunica la regla de nombrado secuencial** (`Referencia-p-01`, `-p-02`…), aunque figure en la guía eMerch, porque transmite una secuencia que ni auditamos ni nos han pedido trasladar. Lo que sí se comunica es **categoría de imagen + mínimo** (lo que la guía exige y sí podemos medir). *(Asunción de dato pendiente de confirmar al integrar: que BQ no expone el orden.)*

**Reparto CSV ↔ PDF (confirmado por el ejemplo del cliente `Export errores - solo ejemplo.csv`):** el **detalle fila-a-fila vive en el CSV, no en el PDF**. El ejemplo es la matriz **sin filtrar** (todos los owners), una fila por error, con columnas `Ref · Producto · Tipo de Error · Ubicación del Error · Diagnóstico del Sistema · Acciones Correctivas · Owner · 1P/3P · Modelo` → **plantilla directa de nuestro CSV** (coincide con las columnas ya definidas). Consecuencia:
- **CSV = mesa de trabajo:** exhaustivo, una fila por error, filtrable/pivotable (plantillas de upload, Jira). El del seller = filtrado a sus refs + Owner:Seller; el interno TIP = Owner:TIP.
- **PDF (seller) = capa de comunicación:** resumen **curado y legible** (cabecera de salud + por referencia + acción + **regla concreta inline**), en el lenguaje/estructura de la guía eMerch (`guia-gua-emerch-60.pdf`: portada con código+nombre de modelo, secciones Designación/Descripción/Atributos/Multimedia, `ATT_xxxx`, mín-máx de imágenes, regla de nombrado `Referencia-p-01`). **No** reproduce fila a fila; para el detalle remite al CSV.
...
6. **Significado de las letras de gama** (A/M/S/C/K/B): qué tier/criterio de surtido representa cada una, **cuántas hay** y si una referencia puede pertenecer a más de una. Necesario para el subtexto del selector de "Gama" y para la lectura del informe.
7. **Atributos obligatorios faltantes — severidad vs. prioridad:** el PDF los trata como **Bloqueante** (pág. 34) pero con prioridad **Media** (Bloque 1). **La pregunta concreta a confirmar:** ¿un atributo obligatorio es *obligatorio para publicar en la plataforma* → **Bloqueante → «No publicable»** (como lo tenemos), o *obligatorio según la guía eMerch* pero la ficha **puede seguir viva** → **fallo de calidad en ficha publicada (Crítica/Leve)**, no «No publicable»? De la respuesta depende en qué caja cae. Nota: **severidad ≠ prioridad** (grave ≠ urgente) — puede ser Bloqueante en severidad y Media en prioridad, son ejes distintos.
8. **Alcance y campos de la consulta a BigQuery** *(lo planteó Susana en la reunión)*: (a) ¿traerá también las referencias que **hoy no aparecen en la web** (no publicadas)? Determina si las **bloqueantes/no-publicadas** entran en el perímetro, en el conteo y en el Health Score — y condiciona el punto 7. (b) ¿Expone el **campo de designación administrativa (ADM)**? Es necesario para los dos checks de ADM ("posible designación administrativa" e "info en ADM no reflejada").
...
11. **¿Health Score calculado o ingerido? ¿y a qué granularidad? ¿y sobre qué universo en el informe del seller?** *(El deck pág. 29 lo sitúa en la cabecera del informe por proveedor junto a SKUs auditados y SKUs conformes → se calcula sobre **todo** el catálogo del proveedor, no solo sobre lo que falla. Nuestro informe solo contiene hallazgos, así que nos faltan el total auditado y las conformes.)* **La granularidad es la pregunta previa:** el valor debe venir **por referencia** — es la unidad de su propio modelo (10 pts) y la única que nos permite componer el HS de cualquier alcance promediando (§2). Si llegara **ya agregado**, los alcances que no coincidan con ese nivel (p. ej. un informe por seller) no tendrían HS. Y además: ¿Lo **computa nuestro módulo** (a partir de los componentes que auditamos) o **viene ya calculado como dato de origen** (su sistema actual de medición de calidad / BigQuery expone el campo)? Determina si solo lo **mostramos** vs. lo **recalculamos**, y se relaciona con la normalización (punto 10) y con los campos de origen (§5.8). *(El PDF pág. 39 sugiere que ya tienen el modelo; confirmar si el valor viene dado o se recalcula.)* **El cliente lo confirma en parte:** *"Nosotros ya lo tenemos normado. Aquí nombro a mi compañero @JAVIER HERNAN porque quizás podamos utilizar su trabajo"* — la normalización existe en su lado, pero reaprovechar ese trabajo está planteado como **posibilidad**, no como entrega comprometida.
...
13. **Modelo de estados de la referencia (4 cajas) — diverge del artefacto en 3 puntos:** (a) severidad **"Mejora" → "Leve"** (naming, para una escala única de gravedad); (b) la **falta esencial** sale de "críticos" y pasa a **"No publicable"** (es Bloqueante = no está en la web); (c) una **contradicción (Crítica)** escala sola a "críticos" aunque sea la única alerta (el artefacto la dejaba en "advertencias" por contar solo cantidad). Se conserva la vía de **acumulación (3+ alertas)**. **Validado con cliente: le gustaron las cajitas** (modelo de 4 estados por gravedad de la peor alerta, con 3+ como vía secundaria). Quedan **bordes menores** por nuestra parte: (a) evitar que la vía «3+ alertas» *solo-leves* infle «críticos»; (b) las fichas **«No publicable»** dependen de que BQ devuelva también las **no publicadas** — asunción: **en BQ podremos filtrar lo que necesitemos; si no, el cliente lo indicará** — y de ahí **cómo cuentan en el Health Score** (arrastran el promedio vs. se excluyen). Ver §2 y §5.8.
...
20. **Escala de criticidad (tipo de error → tier) — SIN DEFINIR.** El cliente: *"detectar las referencias que carecen de información en base a las guías y **establecer una escala de criticidad**"*. Es el **núcleo** del módulo: hoy nuestra severidad (Bloqueante/Crítica/Leve) es **propuesta**, no validada. Además: **publicar = oferta + imagen** (regla de plataforma; los atributos/designación/descripción NO bloquean), y **despublicar es un gesto MANUAL** del emerchand/TIP → la 4ª caja **mantiene el nombre "No publicable"** pero se lee como **recomendación** (publicada que no debería estarlo; despublicar es manual), no como estado automático de plataforma. **Hay fichas publicadas que no deberían estarlo: detectarlas es el valor del módulo.** Falta cerrar: qué tipos de error caen en cada tier y cuáles llegan a "candidata a despublicar".
