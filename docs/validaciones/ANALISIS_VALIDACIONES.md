...
3. Al marcar **falso positivo** se piden **uno o varios motivos** (**multiselección, contextuales al tipo de alerta**) + un **comentario libre** (siempre disponible). Cada motivo mapea con **cómo aprende el motor** (p. ej. color→ampliar sinónimos, ortografía→whitelist, atributo→excepción por categoría). La alerta **sale del informe** y el caso entra en una **cola hacia Configuración del motor**. El motivo es **editable** desde la propia sección de falsos positivos (toggle), y el falso positivo se puede **restaurar**. *(Las etiquetas concretas de motivo por tipo son propuesta a validar con cliente — ver §5.)*
