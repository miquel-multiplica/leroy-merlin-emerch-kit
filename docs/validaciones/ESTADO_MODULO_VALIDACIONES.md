...
- **Configuración del motor** (admin): **enlace** (verde, icono ajustes) junto a "Nueva auditoría" → **panel "Reglas del motor de validaciones"** (pantalla propia con navbar y ancho completo):
  ...
  - **Versionado del motor** (barra superior): badge **"Motor vX · publicado …"**; si hay cambios sin publicar → chip ámbar **"Cambios sin publicar · N niveles"** + **"Ver detalles"** (modal-lista de los niveles editados, clicables), y en el árbol los niveles afectados salen con **pelotita naranja + negrita**. **"Publicar nueva versión"** (oculto si no hay cambios) abre un **diálogo para nombrar la versión**; se registra con **título + autor**. **"Ver historial"** (icono reloj) → **drawer lateral derecho** con las versiones (badge · título · fecha · autor · **Restaurar**) y un **dropdown** que lista con bullets **los niveles tocados** en cada versión.
  ...
  - **Sección "Vocabulario y listas"** (entrada propia en la navegación, hermana de "Prompt general"): listas **transversales** con chips editables (añadir/quitar) — **excepciones ortográficas**, **colores y equivalencias** (pares término→canónico), **materiales**, **abreviaturas y símbolos**. Se engancha al **versionado** como un nivel más (pelotita naranja, entra en "cambios sin publicar" y en el historial). *Contenido de las listas = ejemplos; el real lo aporta el cliente/motor.*
