...
- **Configuración del motor** (admin): **enlace** (verde, icono ajustes) junto a "Nueva auditoría" → **panel "Reglas del motor de validaciones"** (pantalla propia con navbar y ancho completo):
  ...
  - *Pendiente de maquetar:* **sección propia de "Vocabulario y listas"** (excepciones ortográficas, tabla de colores, materiales, abreviaturas) — recomendada como **entrada aparte** en la navegación del panel (dato transversal y estructurado, no encaja en el árbol por niveles). Los **prompts reales** por nivel también quedan pendientes (el proto usa texto de relleno) — *no prioritario ahora*.
