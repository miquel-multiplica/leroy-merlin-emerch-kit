...
- **Detalle de auditoría** (`v-informe`, una sola pantalla condicional por estado):
  ...
  - **Pendiente de revisión / Revisada**:
    ...
    - **Falsos positivos** (la **IA no los prescribe: los detecta el humano**): al marcar se piden **motivos contextuales al tipo** (chips **multiselección**) + **comentario**. Los checks de IA llevan un **badge IA** (indica el origen de la alerta, no un veredicto). **En bloque (Familia → Modelo → Referencia):** si la alerta comparte **disparador** con otras, un botón **"Ver y aplicar por modelo"** abre una **megamodal** que lista los **modelos afectados** con sus referencias/columnas; el revisor **selecciona por modelo** (arrastra sus refs), excluye los que no aplican y afina por referencia (*"N modelos · M referencias"*). Salen del informe al cajón de FP con **Motivo / Editar / Restaurar** (**restaurar** un bloque pregunta si deshacer las demás). **Hipótesis de elegibilidad** (no confirmada, ver pendiente 16): checks de **juicio** (discrepancias, ortografía, SEO, administrativa) llevan botón de FP; **ausencias objetivas** (sin descripción/designación, atributos vacíos, imágenes, longitud) no.
